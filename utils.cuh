...
template<const int BM, const int BN, const int BK>
__device__ void load_tiles_gmem_to_smem_vectorize(float *ga, float *gb, float *sa, int row_stride_sa, float *sb, int row_stride_sb, int phase_idx, int N, int K) {
    ...
    for (int load_phase = 0 ; load_phase < cells_per_thread_A / 4 ; ++load_phase) {
        ...
        sa[(col_in_tile + 0) * BM + row_in_tile] = tmp.x;
        sa[(col_in_tile + 1) * BM + row_in_tile] = tmp.y;
        sa[(col_in_tile + 2) * BM + row_in_tile] = tmp.z;
        sa[(col_in_tile + 3) * BM + row_in_tile] = tmp.w;
    }
    ...
    for (int load_phase = 0 ; load_phase < cells_per_thread_B / 4 ; ++load_phase) {
        ...
        int row_global = row_in_B + row_in_tile;
        int col_global = col_in_B + col_in_tile;
        *reinterpret_cast<float4*>(&sb[row_in_tile * row_stride_sb + col_in_tile]) =
